...
class FreezableConv2dFunction(torch.autograd.Function):
    ...
    def backward(ctx: torch.Any, grad_output: torch.Any) -> torch.Any:
        ...
        if ctx.needs_input_grad[1]:
            if len(ctx.freeze) > 0:
                if ctx.groups == weight.shape[0] and ctx.groups == weight.shape:
                ...
                elif ctx.groups != 1: #TODO Depthwise convolution with out_channels = input_channels and other sizes of groups will use false sparse backward
